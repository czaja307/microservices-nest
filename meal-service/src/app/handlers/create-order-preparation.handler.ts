...
import {
  OrderPreparation,
  PreparationStatus,
} from '../../domain/entities/order-preparation.entity';
...
export class CreateOrderPreparationHandler
  implements ICommandHandler<CreateOrderPreparationCommand>
{
  ...
  async execute(
    command: CreateOrderPreparationCommand,
    ...
    const maxPrepTime = Math.max(
      ...meals.map((meal) => meal.preparationTimeMinutes),
      0,
    );
    ...
    this.logger.log(
      `Order preparation started for order ID: ${savedOrderPreparation.orderId}`,
    );
